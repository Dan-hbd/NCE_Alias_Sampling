...
class NCELoss(nn.Module):
    ...
    def __init__(self,
        ...
        if norm_term == 'auto':
            # numel() tensor中元算个数总和
            print("noise中元素的个数和", noise.numel())
    ...
    def forward(self, target, input, *args, **kwargs):
    ...
    def _get_logit(self, target_idx, noise_idx, *args, **kwargs):
        ...
        """
        # target_logit: [bsz, 1]   noise_logit: [bsz, 1, noise_sample_number]
        target_logit, noise_logit = self.get_score(target_idx, noise_idx, input, *args, **kwargs)
        # 减去 norm_term
    ...
    def nce_loss(self, logit_target_in_model, logit_noise_in_model, logit_noise_in_noise, logit_target_in_noise):
        ...
        # logit_target_in_model [bsz, 1] => [bsz, 1, 1]  logit_noise_in_model: [bsz, 1, noise_sample_num]
        ...
        # 希望分类到target的概率为1， 分类到nosie 的结果是0(否)
        label[:, :, 0] = 1
