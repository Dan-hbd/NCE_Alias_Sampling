...
class IndexLinear(NCELoss):
    ...
    def _compute_sampled_logit_batched(self, target_idx, noise_idx, input):
        ...
        # flatten the following matrix
        input = input.contiguous().view(-1, input.size(-1))  # [bsz, emb_dim]
        target_idx = target_idx.view(-1)
        noise_idx = noise_idx[0, 0].view(-1)  # 所以batch中的每一个样本所选择的负样本都是一样的

        target_batch = self.emb(target_idx)  # 我们任务中的target index 应该是根据我们自己的情况，他这里不知道为什么全是1
        ...
        noise_batch = self.emb(noise_idx)  # Nr X H 对负样本做embedding， batch中所有元素用的一样的负样本
